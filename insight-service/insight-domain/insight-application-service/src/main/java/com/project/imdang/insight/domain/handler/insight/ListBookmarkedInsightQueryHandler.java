...
@Component
public class ListBookmarkedInsightQueryHandler {
    ...
    public Page<InsightResult> listBookmarkedInsight(ListBookmarkedInsightQuery listBookmarkedInsightQuery) {
        ...
        PageRequest pageRequest = PagingUtils.getPageRequest(
                listBookmarkedInsightQuery.getPageNumber(), listBookmarkedInsightQuery.getPageSize(), listBookmarkedInsightQuery.getDirection(), listBookmarkedInsightQuery.getProperties());
        MemberId memberId = listBookmarkedInsightQuery.getMemberId();
        Boolean onlyMine = listBookmarkedInsightQuery.getOnlyMine();

        Page<Insight> paged;
        //단지별 보기 선택 시
        if (listBookmarkedInsightQuery.getApartmentComplexName() != null) {
            // 단지
            ApartmentComplex apartmentComplex = new ApartmentComplex(listBookmarkedInsightQuery.getApartmentComplexName());
        ...
        // 전체 선택 시 (단지별 보기가 아닌 경우)
            ...
            //내 인사이트만 보기 활성화 시
            if (Boolean.TRUE.equals(onlyMine)) {
                paged = insightRepository.findAllByMemberIdAndDistrictAndOnlyMine(memberId, listBookmarkedInsightQuery.getDistrict(), pageRequest);
            }
            // 내 인사이트만 보기 비활성화 시
            else {
                paged = insightRepository.findAllByMemberIdAndDistrict(memberId, listBookmarkedInsightQuery.getDistrict(), pageRequest);
            }
        }
